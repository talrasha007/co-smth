...
module.exports = function (token, appid, secret, signature) {
    ...
    this.sns = {
        getReferCount: function *(mode) {
            return yield *post('refer/count.json', { mode: mode });
        },

        loadRefer: function *(mode, from, size) {
            return yield *post('refer/load.json', {
                mode: mode,
                from: from,
                size: size
            });
        ...
        setReferRead: function *(mode, position) {
            return yield *post('refer/read.json', {
                mode: mode,
                position: position
            });
        },

        loadTimeLine: function *(loadold, oldid, size, order) {
            return yield *post('timeline/boards_threads.json', {
                oldid: oldid,
                size: size,
                loadold: loadold,
                order: order
    ...
    this.guess = {
        listGuess: function *() {
            return yield *post('guess/allguess.json', {});
        },

        loadGuess: function *(guessId) {
            return yield *post('guess/show_guess.json', { guessid: guessId});
        },
        
        listMatch: function *() {
            return yield *post('guess/show_allmatch.json', {});
        },

        listGuessTop: function *(guessId) {
            return yield *post('guess/guess_top.json', { guessid: guessId });
        },

        regGuess: function *(guessId, realName, phone) {
            return yield *post('guess/reg_guess.json', {
                guessid: guessId,
                realname: realName,
                phone: phone
            });
        ...
        loadMatch: function *(guessId, matchId) {
            return yield *post('guess/show_match.json', {
                guessid: guessId,
                matchid: matchId
            });
        },

        voteMatch: function *(guessId, matchId, sel, money) {
            return yield *post('guess/vote.json', {
                guessid: guessId,
                matchid: matchId,
                sel: sel,
                money: money
